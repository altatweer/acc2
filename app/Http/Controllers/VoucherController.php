   ...
   public function store(Request $request)
   {
       ...
       DB::transaction(function () use ($validated, $request) {
           ...
           foreach ($validated['transactions'] as $tx) {
               ...
               // المهم: نحفظ نفس سعر الصرف في كلا السطرين، ولكن في العرض نعرضه فقط في السطر الأول
               // السطر بعملة الصندوق: إذا كانت نفس العملة الأساسية، استخدم 1.0 (لن يظهر)
               // وإلا استخدم سعر الصرف المدخل (سيظهر في العرض)
               $cashExchangeRate = ($cashCurrency === $baseCurrency) ? 1.0 : $exchangeRate;
               // السطر بعملة الهدف: نحفظ نفس سعر الصرف (للاستخدام في العرض)
               // في صفحة العرض، سنعرضه فقط في السطر الأول
               $targetExchangeRate = ($targetCurrency === $baseCurrency) ? 1.0 : $exchangeRate;
