...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-8">
                <h1><i class="fas fa-file-invoice-dollar mr-2"></i>تعديل فاتورة</h1>
                <p class="text-muted">تعديل فاتورة موجودة مع دعم العملات المتعددة</p>
            </div>
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="form-group col-md-3">
                            ...
                            <input type="text" name="invoice_number" id="invoice_number" class="form-control" value="{{ old('invoice_number', $invoice->invoice_number) }}" placeholder="سيتم إنشاؤه تلقائياً">
                        </div>
                        <div class="form-group col-md-3">
                        ...
                        <div class="form-group col-md-2">
                            ...
                            <select name="currency" id="currency" class="form-control select2-currency" required>
                                @foreach($currencies as $curr)
                                    <option value="{{ $curr->code }}" {{ old('currency', $invoice->currency) == $curr->code ? 'selected' : '' }}>
                                        {{ $curr->code }} - {{ $curr->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-md-2">
                            ...
                            <input type="hidden" name="exchange_rate" id="exchange_rate" value="{{ old('exchange_rate', $invoice->exchange_rate) }}">
            ...
            <div class="card shadow mb-4">
                <div class="card-body text-center bg-light">
                    ...
                    <h3 class="mb-0" id="invoice_total_display">
                        <span id="total_amount">{{ number_format($invoice->total, 2) }}</span>
                        <span id="total_currency"></span>
                    </h3>
                    <input type="hidden" name="total" id="invoice_total" value="{{ $invoice->total }}">
                </div>
            </div>
                        ...
                        <table class="table table-bordered table-hover" id="itemsTable">
                            ...
                            <tbody>
                                ...
                                <tr data-item-index="{{ $index }}">
                                    ...
                                    <td>
                                        <select name="items[{{ $index }}][item_id]" class="form-control select2-item" data-index="{{ $index }}" required>
                                    ...
                                    <td>
                                        <input type="number" name="items[{{ $index }}][quantity]" class="form-control item-quantity text-center" step="1" min="1" placeholder="1" value="{{ $item->quantity }}" required>
                                    </td>
                                    <td>
                                        <input type="number" name="items[{{ $index }}][unit_price]" class="form-control item-price text-right" step="0.01" min="0" placeholder="0.00" value="{{ $item->unit_price }}" required>
                                    </td>
                                    <td>
                                        <input type="number" name="items[{{ $index }}][line_total]" class="form-control item-total text-right bg-light font-weight-bold" step="0.01" value="{{ $item->line_total }}" readonly>
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-danger btn-sm remove-item" {{ $invoice->invoiceItems->count() <= 1 ? 'disabled' : '' }}>
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover" id="expenseTable">
                            ...
                            <tbody>
                                @php
                                    $expenseAttachment = $invoice->expenseAttachments()->first();
                                    $expenseLines = $expenseAttachment ? $expenseAttachment->lines : collect();
                                @endphp
                                ...
                                @else
                                @foreach($expenseLines as $expIndex => $line)
                                <tr data-expense-index="{{ $expIndex }}">
                                    <td class="text-center">{{ $expIndex + 1 }}</td>
                                    <td>
                                        <select name="expense_attachment_lines[{{ $expIndex }}][cash_account_id]" class="form-control select2-cash-account" data-index="{{ $expIndex }}">
                                            <option value="">-- اختر الحساب --</option>
                                            @foreach(\App\Models\Account::where('is_cash_box', 1)->get() as $acc)
                                                <option value="{{ $acc->id }}" data-code="{{ $acc->code }}" data-name="{{ $acc->name }}" {{ $line->cash_account_id == $acc->id ? 'selected' : '' }}>
                                                    {{ $acc->code ? $acc->code . ' - ' . $acc->name : $acc->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>
                                        <select name="expense_attachment_lines[{{ $expIndex }}][expense_account_id]" class="form-control select2-expense-account" data-index="{{ $expIndex }}">
                                            <option value="">-- اختر الحساب --</option>
                                            @foreach(\App\Models\Account::where('is_group', 0)->where('is_cash_box', 0)->get() as $acc)
                                                <option value="{{ $acc->id }}" data-code="{{ $acc->code }}" data-name="{{ $acc->name }}" {{ $line->expense_account_id == $acc->id ? 'selected' : '' }}>
                                                    {{ $acc->code ? $acc->code . ' - ' . $acc->name : $acc->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>
                                        <input type="number" name="expense_attachment_lines[{{ $expIndex }}][amount]" class="form-control text-right" step="0.01" min="0" placeholder="0.00" value="{{ $line->amount }}">
                                    </td>
                                    <td>
                                        <select name="expense_attachment_lines[{{ $expIndex }}][currency]" class="form-control select2-currency-small">
                                            @foreach($currencies as $curr)
                                                <option value="{{ $curr->code }}" {{ $line->currency == $curr->code ? 'selected' : '' }}>
                                                    {{ $curr->code }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <td>
                                        <input type="text" name="expense_attachment_lines[{{ $expIndex }}][description]" class="form-control" placeholder="وصف المصروف" value="{{ $line->description }}">
                                    </td>
                                    <td>
                                        <button type="button" class="btn btn-danger btn-sm remove-expense" {{ $expenseLines->count() <= 1 ? 'disabled' : '' }}>
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 text-right">
                            <button type="submit" class="btn btn-success btn-lg" id="submitBtn">
                                <i class="fas fa-save mr-1"></i>تحديث الفاتورة
                            </button>
                            <a href="{{ route('invoices.show', $invoice) }}" class="btn btn-secondary">
...
<script>
let itemIndex = {{ $invoice->invoiceItems->count() }};
let expenseIndex = {{ \App\Models\Setting::get('enable_invoice_expense_attachment', false) && $invoice->expenseAttachments()->first() ? $invoice->expenseAttachments()->first()->lines->count() : 1 }};
...
$(document).ready(function(){
    ...
    $('#invoiceForm').on('submit', function(e) {
        ...
        $('#submitBtn').prop('disabled', true)
            .html('<i class="fas fa-spinner fa-spin mr-1"></i>جاري التحديث...');
    });
