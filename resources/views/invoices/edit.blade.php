...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0">تعديل الفاتورة {{ $invoice->invoice_number }}</h1>
      </div>
      <div class="col-sm-6 text-left">
        <a href="{{ route('invoices.show', $invoice) }}" class="btn btn-secondary">العودة للفاتورة</a>
      ...
      <div class="col-lg-10 offset-lg-1 col-md-12">
        <div class="card card-warning">
          <div class="card-header">
            <h3 class="card-title">بيانات الفاتورة - تعديل</h3>
          </div>
          <form action="{{ route('invoices.update', $invoice) }}" method="POST">
            ...
            <div class="card-body">
              ...
              <div class="form-row">
                <div class="form-group col-md-4">
                  ...
                  <input type="date" name="date" class="form-control" value="{{ old('date', $invoice->date->format('Y-m-d')) }}" required>
                </div>
                <div class="form-group col-md-4">
                  <label>العملة</label>
                  <select name="currency" id="currency" class="form-control select2" required>
                    ...
                    @foreach($currencies as $cur)
                      <option value="{{ $cur->code }}" {{ old('currency', $invoice->currency) == $cur->code ? 'selected' : '' }}>{{ $cur->code }} - {{ $cur->name }}</option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group col-md-4">
              ...
              <div class="row mb-3">
                <div class="col text-right">
                  <h4>إجمالي الفاتورة: <span id="invoice_total_display" class="badge badge-primary p-2" style="font-size:1.2rem;">{{ number_format($invoice->total, 2) }}</span></h4>
                </div>
              </div>
              ...
              <div class="table-responsive">
                <table class="table table-bordered" id="invoice_items_table">
                  ...
                  <tbody id="invoice_items_body">
                    ...
                    <tr>
                      <td>
                        <select name="items[{{ $index }}][item_id]" class="form-control select2 items-select" required>
                          <option value="" disabled>-- اختر المنتج/الخدمة --</option>
                          @foreach($items as $itm)
                            <option value="{{ $itm->id }}" data-price="{{ $itm->unit_price }}" 
                              {{ $invoiceItem->item_id == $itm->id ? 'selected' : '' }}>
                              {{ $itm->name }} ({{ $itm->type }})
                            </option>
                          @endforeach
                        </select>
                      </td>
                      <td><input type="number" name="items[{{ $index }}][quantity]" value="{{ $invoiceItem->quantity }}" class="form-control item-quantity" min="1" step="1" required></td>
                      <td><input type="number" name="items[{{ $index }}][unit_price]" value="{{ $invoiceItem->unit_price }}" class="form-control item-price" step="0.01" required></td>
                      <td><input type="number" name="items[{{ $index }}][line_total]" value="{{ $invoiceItem->line_total }}" class="form-control item-total" step="0.01" readonly></td>
            ...
            <div class="card-footer text-right">
              <button type="submit" class="btn btn-warning">تحديث الفاتورة</button>
...
$(function(){
    ...
    // Update line total when item is selected
